      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className='sr-only'>
            <SheetTitle>{t('sidebarUi.mobileTitle')}</SheetTitle>
            <SheetDescription>{t('sidebarUi.mobileDesc')}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className='sr-only'>{t('sidebarUi.toggle')}</span>
    </Button>
  )
}
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      aria-label={t('sidebarUi.toggle')}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={t('sidebarUi.toggle')}
      className={cn(
