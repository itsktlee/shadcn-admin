...
export function UsersMultiDeleteDialog<TData>({
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: t('users.dialogs.bulkDelete.loading'),
      success: () => {
        setValue('')
        table.resetRowSelection()
        return t('users.dialogs.bulkDelete.success', {
      ...
      title={
        <span className='text-destructive'>
          <AlertTriangle
            ...
            size={18}
          />{' '}
          {t('users.dialogs.bulkDelete.title', {
            count: selectedRows.length,
            itemLabel:
              selectedRows.length > 1
                ? t('users.dialogs.bulkDelete.itemPlural')
                : t('users.dialogs.bulkDelete.itemSingular'),
          })}
        </span>
      }
      desc={
        <form
          ...
          className='space-y-4'
        >
          <p className='mb-2'>{t('users.dialogs.bulkDelete.desc')}</p>
